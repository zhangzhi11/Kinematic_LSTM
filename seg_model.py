# -*- coding: utf-8 -*-
"""
Created on Mon Jan 02 21:13:35 2017
...
#    b_idx = 0
    ...
    #        file_label = labels[file_idx][:-1]
        ...
        yield [batch_feat[:,:,:33],batch_feat[:,:,33:66],batch_feat[:,:,66:]], batch_label_cate
...
def build_model(n_feat = 528, n_class = 21):
    n_hidden = 200
    p_in = Input(shape = (None,int(n_feat/3)), name = 'p_in')
    v_in = Input(shape = (None,int(n_feat/3)), name = 'v_in')
    a_in = Input(shape = (None,int(n_feat/3)), name = 'a_in')

    dense_p = TimeDistributed(Dense(output_dim = n_hidden, activation='relu'))(p_in)    
    dense_v = TimeDistributed(Dense(output_dim = n_hidden, activation='relu'))(v_in)   
    dense_a = TimeDistributed(Dense(output_dim = n_hidden, activation='relu'))(a_in)       
#    drop_dense1 = Dropout(0.2)(dense1)
    
    f_lstm_p = LSTM(output_dim = n_hidden, return_sequences = True)(dense_p)
    b_lstm_p = LSTM(output_dim = n_hidden,return_sequences = True,go_backwards = True)(dense_p)
    lstm_p = merge([f_lstm_p, b_lstm_p], mode='concat')    
    
    f_lstm_v = LSTM(output_dim = n_hidden, return_sequences = True)(dense_v)
    b_lstm_v = LSTM(output_dim = n_hidden,return_sequences = True,go_backwards = True)(dense_v)
    lstm_v = merge([f_lstm_v, b_lstm_v], mode='concat') 

    f_lstm_a = LSTM(output_dim = n_hidden, return_sequences = True)(dense_a)
    b_lstm_a = LSTM(output_dim = n_hidden,return_sequences = True,go_backwards = True)(dense_a)
    lstm_a = merge([f_lstm_a, b_lstm_a], mode='concat') 
    
    lstm1 = merge([lstm_p, lstm_v, lstm_a], mode='concat') 
...
#    lstm3 = merge([f_lstm3, b_lstm3], mode='concat')  
    ...
    dense2 = TimeDistributed(Dense(output_dim = n_hidden, activation='relu'))(lstm2)  
    
    output = TimeDistributed(Dense(output_dim = n_class, activation='softmax'))(dense2)
    
    model = Model(input = [p_in, v_in ,a_in], output = output)    
...
#                valid_y = valid_y[:-1]
            pred_y = model.predict_on_batch([valid_x[:,:,:33],valid_x[:,:,33:66],valid_x[:,:,66:]])
...
#    class_weight = []
    for epoch_idx in range(n_epoch):
        ...
        if valid_loss>=best_loss:
            save_model(model,name = data_name+'_'+model_name+'_seg')
...
if __name__ == '__main__':
    ...
    model_name = 'D200_2BLSTM200_2D200'
    data_name = 'sk_pva_99'
    ...
    print 'Build model...'
    model = build_model(n_feat = 99, n_class = 21)
...
#    print 'Test...'
#    jaccards,cnf_matrix = test_model(model, labels, data_name = 'sk_pva_99')   
